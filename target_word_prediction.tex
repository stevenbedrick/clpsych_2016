...
We used the the OpenGrm-NGram language modeling toolkit \cite{RoarkOpenGRM} to build the language models themselves, using an n-gram order of 4, with Kneser-Ney smoothing \cite{Kneser:1995ib}. 
...
As an example of one of the AphasiaBank rules, monosyllabic productions with an onset, vowel nucleus, and coda must match two of the three elements of a target word in order to be considered phonologically similar \cite{MacWhinney:2000aa}.
...
We first convert our best-guess orthographic representation of the subject's non-lexical production to an estimated phonological representation using the Phonetisaurus grapheme-to-phoneme toolkit \cite{Novak:2011aa}. 
...
As far back as Shannon's word-guessing game \cite{Shannon:1951p5641}, researchers have sought to leverage the statistical regularities in natural language to predict missing or subsequent words. 
