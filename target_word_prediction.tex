...
According to the National Institute of Neurological Disorders and Stroke, approximately 1,000,000 people in the United States suffer from aphasia.
Aphasia is a common consequence of stroke: prevalence estimates for aphasia among stroke patients vary, but are typically in the neighborhood of 30\% \cite{Engelter:2006}.
...
% I'd say Shattuck-Hufnagel's work on this is where I'd start.
%
...
%  Volume = {3},
%  Year = {1986}}
% But I think I'd just cut this digression altogether. --KG.
...
That said, it is worth nothing that the standard model explaining these sorts of anomic errors is Dell's two-step word production model \cite{Dell:1986,Dell:1997}.
...
Computational analysis of aphasic speech:
    ...
    automatic sub-typing from narrative transcripts\cite{Fraser:2014a}
    statistical parsing to detect\cite{Fraser:2014a}
        Syntactic analysis of same: \cite{Goodglass:1994}
Segmentation of aphasic speech: \cite{Fraser:2015}

Guessing the next word: \cite{Shannon:1951}

Using phonemic similarity to identify potential lexical items: \cite{Han:2011,Choudhury:2007}
    - much related work in text normalization \cite{Sproat:2001}
...
We used the the OpenGrm-NGram language modeling toolkit \cite{Roark:2012} to build the language models themselves, using an n-gram order of 4, with XXX smoothing. % TODO: Joel, what kind of smoothing did we use?
...
As an example of one such rule, under the guidelines used by the AphasiaBank project, monosyllabic productions with an onset, vowel nucleus, and coda must match two of the three elements of a target word in order to be considered phonologically similar \cite{MacWhinney:2000}.
...
We first convert our best-guess orthographic representation of the subject's non-lexical production to an estimated phonological representation using the Phonetisaurus grapheme-to-phoneme library \cite{Novak:2012}.
...
% TODO(steve): delete the other bib files and rename kbg_refs to something else.
